...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($product_id_err) && empty($seed_amount_kg_err) && empty($sowing_date_err) && empty($field_name_err)) {
        ...
        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Generate unique production code with retry mechanism
            $max_retries = 5;
            $production_code = null;
            
            for ($i = 0; $i < $max_retries; $i++) {
                $production_code = generateFarmProductionId();
                ...
                if ($i == $max_retries - 1) {
                    $production_code = 'FP' . date('y') . str_pad(mt_rand(100, 999), 3, '0', STR_PAD_LEFT);
                }
            }
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                error_log("Error creating farm production: " . $error_msg);
            }
